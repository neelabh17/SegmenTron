...
class VOCContextSegmentation(SegmentationDataset):
    """Pascal VOC Semantic Segmentation Dataset.
    ...
    def __init__(self, root='datasets/VOCdevkit', split='train', mode=None, transform=None, **kwargs):
        super(VOCContextSegmentation, self).__init__(root, split, mode, transform, **kwargs)
        _voc_root = os.path.join(root, self.BASE_DIR)
        _mask_dir = os.path.join(_voc_root, 'SegmentationClassContext')

        _image_dir = os.path.join(_voc_root, 'JPEGImages')

        # train/val/test splits are pre-cut
        _splits_dir = os.path.join(_voc_root, 'ImageSets/SegmentationContext')
        if split == 'train':
