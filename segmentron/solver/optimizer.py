...
def _get_paramters_mmseg(model):
    """mmSeg version"""
    params_list = list()
    if hasattr(model, 'backbone') and model.backbone is not None and hasattr(model, 'decoder'):
        params_list.append({'params': model.backbone.parameters(), 'lr': cfg.SOLVER.LR})
        if cfg.MODEL.BN_EPS_FOR_ENCODER:
            logging.info('Set bn custom eps for bn in encoder: {}'.format(cfg.MODEL.BN_EPS_FOR_ENCODER))
            _set_batch_norm_attr(model.backbone.named_modules(), 'eps', cfg.MODEL.BN_EPS_FOR_ENCODER)
...
def get_optimizer_mmseg(model):
    """MMseg version"""
    parameters = _get_paramters_mmseg(model)
    opt_lower = cfg.SOLVER.OPTIMIZER.lower()
